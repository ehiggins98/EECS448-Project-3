...
train_file_names = [f for f in listdir('.') if isfile(join('.', f)) and re.compile("train.+\.tfrecord").match(f)]
dev_file_names = ['dev.tfrecord']
...
def process(tensor):
    max_angle = tf.constant(0.349066) # 20 degrees in radians
